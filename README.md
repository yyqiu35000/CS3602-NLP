...
## 注：提升复现稳定性的建议
我们经过多次尝试发现加速性能确实相当的不稳定，虽然avg attn波动较小，但TTFT、TPOT、Throughput等指标波动较大（因为我们测试环境中存在大量额外开销，如反量化，top-k匹配等，这些似乎会明显的受到电脑性能的影响）。因此我们尝试过下列方案    
...
但虽然进行了上述方案，性能仍然存在波动，会出现可能StreamingLLM速度不如baseline，集成方案不如baseline等情况，如果遇到请多尝试几次，一般情况下StreamingLLM的速度会快于baseline，集成方案的速度也会大致与baseline接近（主要优化在显存开销）。
